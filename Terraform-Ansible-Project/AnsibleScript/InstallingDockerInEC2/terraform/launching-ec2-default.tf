...
#Local provisioner for running ansible script for configuration 
resource "null_resource" "ansible_test" {
  provisioner "local-exec" {
    working_dir = "../"
    command     = "ansible-playbook --inventory ${aws_instance.cn-vm.public_ip}, --private-key ${var.private_key_location} --user ec2-user docker-provising.yaml"
  }
}